...
@interface ZJNNewViewController () <UIScrollViewDelegate>
/** 内容标签 */
...
@implementation ZJNNewViewController
#pragma mark - 初始化标签数据
- (NSArray *)tags {
    if (!_tags) {
        _tags = @[@"全部",@"视频",@"图片",@"段子",@"声音"];
    }
    return _tags;
}
- (NSDictionary *)contentType {
    if (!_contentType) {
        _contentType = @{@"全部":@(1),@"视频":@(41),@"图片":@(10),@"段子":@(29),@"声音":@(31)};
    }
    return _contentType;
}
- (void)viewDidLoad {
...
- (void)mainTagButtonClick {
    ZJNAllTagViewController *mainTag = [[ZJNAllTagViewController alloc] init];
...
- (void)setUpContentViewControllers {
    for (NSString *title in self.tags) {
        ZJNNewContentViewController *contentController = [[ZJNNewContentViewController alloc] init];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    ZJNNewContentViewController *contentController = self.childViewControllers[index];
